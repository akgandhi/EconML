...
df_summ<- df %>% 
  group_by(market) %>% 
  summarize(`Number of Models` = n(),
            `Horsepower/Weight` = mean(hpwt),
            `Space` = mean(space),
            `AC Dummy` = mean(air),
            `Miles per Dollar` = mean(mpd),
            `Miles per Gallon` = mean(mpg),
            `Price, Thousands of 1983 Dollars` = mean(price),
            `Quantity` = mean(quantity),
            `Produced in Japan` = mean(japan),
            `Produced in Europe` = mean(euro)
            ) %>% 
  ungroup
