...
Finally, we observe whether the participation to some social program is correlated with the participation to other programs too.
...
cpsSamplePov$social <- NA
cpsSamplePov$social <- ifelse(cpsSamplePov$pubhous==2,1,cpsSamplePov$social)
cpsSamplePov$social <- ifelse(cpsSamplePov$rentsub==2,2,cpsSamplePov$social)
cpsSamplePov$social <- ifelse(cpsSamplePov$heatsub==2,3,cpsSamplePov$social)
cpsSamplePov$social <- ifelse(cpsSamplePov$foodstmp==2,4,cpsSamplePov$social)
cpsSamplePov$social <- ifelse(cpsSamplePov$lunchsub==2,5,cpsSamplePov$social)
cpsSamplePov$social <- ifelse(is.na(cpsSamplePov$social),0,cpsSamplePov$social)

# labels <- c(0 == "No Programs", 1 == "Public Housing",
#   2 == "Rent Subsidy",
#   3 == "Heat Subsidy",
#   4 == "Food Stamps",
#   5 == "Lunch Subsidy"
...
# }
# 
# levels(cpsSamplePov$social)[levels(cpsSamplePov$social)==0] <- "No Programs"
# levels(cpsSamplePov$social)[levels(cpsSamplePov$social)==1] <- "Public Housing"
# levels(cpsSamplePov$social)[levels(cpsSamplePov$social)==0] <- "Rent Subsidy"
# levels(cpsSamplePov$social)[levels(cpsSamplePov$social)==0] <- "Heat Subsidy"
# levels(cpsSamplePov$social)[levels(cpsSamplePov$social)==0] <- "Food Stamps"
# levels(cpsSamplePov$social)[levels(cpsSamplePov$social)==0] <- "Lunch Subsidy"

NIUNoYes <- c("Not in Universe","No","Yes")
p1 <- ggplot(cpsSamplePov, 
       aes(x=as.factor(pubhous))) + 
       geom_bar(alpha = .5, aes(fill=..count..)) +
       scale_fill_gradient("count", low="green", high="red") + 
       scale_x_discrete(labels= NIUNoYes) + 
       ylab("Living in Public Housing") + 
       theme(legend.position="none", axis.title.x=element_blank()) +
       facet_wrap(~cpsSamplePov$social)

p2 <- ggplot(cpsSamplePov, 
       aes(x=as.factor(rentsub))) + 
       geom_bar(alpha = .5, aes(fill=..count..)) +
       scale_fill_gradient("count", low="green", high="red") + 
       scale_x_discrete(labels= NIUNoYes) + 
       ylab("Government Rent Subsidy Recipient") + 
...
p3 <- ggplot(cpsSamplePov, 
       aes(x=as.factor(heatsub))) + 
       geom_bar(alpha = .5, aes(fill=..count..)) +
       scale_fill_gradient("count", low="green", high="red") + 
       scale_x_discrete(labels= NIUNoYes) + 
       ylab("Government Heat Subsidy Recipient") + 
...
p4 <- ggplot(cpsSamplePov, 
       aes(x=as.factor(foodstmp))) + 
       geom_bar(alpha = .5, aes(fill=..count..)) +
       scale_fill_gradient("count", low="green", high="red") + 
       scale_x_discrete(labels= NIUNoYes) + 
       ylab("Government Food Stamps Recipient") + 
